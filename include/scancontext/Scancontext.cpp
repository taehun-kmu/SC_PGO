...
MatrixXd SCManager::makeScancontext(pcl::PointCloud<SCPointType> & _scan_down)
{
  ...
  for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++) {
    ...
    ring_idx = std::max(
      std::min(
        PC_NUM_RING, int(ceil(
          (azim_range / PC_MAX_RADIUS) * PC_NUM_RING)) ), 1);
    sctor_idx = std::max(
      std::min(
        PC_NUM_SECTOR, int(ceil(
          (azim_angle / 360.0) * PC_NUM_SECTOR)) ), 1);
  ...
  Eigen::MatrixXd & _curr_desc)
{
  ...
  polarcontext_tree_batch_->index->findNeighbors(
    knnsearch_result, &curr_key[0] /* query */, nanoflann::SearchParams(
      10) );                                                                                                                   // error here
    ...
    candidate_iter_idx++)
  {
    ...
    std::pair<double,
      int> sc_dist_result = distanceBtnScanContext(curr_desc, polarcontext_candidate);
